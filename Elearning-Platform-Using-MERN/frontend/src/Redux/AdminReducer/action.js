...
export const getProduct = (page, limit, search, order) => (dispatch) => {
  dispatch({ type: PRODUCT_REQUEST })
  axios.get(`${API_BASE_URL}/courses?page=${page}&limit=${limit}&q=${search}&sortBy=price&sortOrder=${order}`, {
    headers: {
      Authorization: `Bearer ${token}`
    }
  }).then((res) => {
...
export const getUser = (page, limit, search, order) => (dispatch) => {
  const currentToken = JSON.parse(localStorage.getItem('user'))?.token || "";
  dispatch({ type: PRODUCT_REQUEST })
  axios.get(`${API_BASE_URL}/users?page=${page}&limit=${limit}`, {
    headers: {
      Authorization: `Bearer ${currentToken}`
    }
  }).then((res) => {
...
export const getvideo = (page, limit, user) => (dispatch) => {
  dispatch({ type: PRODUCT_REQUEST })
  axios.get(`${API_BASE_URL}/videos?page=${page}&limit=${limit}&user=${user}`, {
    headers: {
      Authorization: `Bearer ${token}`
    }
  }).then((res) => {
...
export const deleteProduct = (id) => (dispatch) => {
  dispatch({ type: PRODUCT_REQUEST });
  axios.delete(`${API_BASE_URL}/courses/delete/${id}`, {
    headers: {
      Authorization: `Bearer ${token}`
    }
  }).then((res) => {
...
export const deleteUsers = (id) => (dispatch) => {
  const currentToken = JSON.parse(localStorage.getItem('user'))?.token || "";
  dispatch({ type: PRODUCT_REQUEST });
  axios.delete(`${API_BASE_URL}/users/delete/${id}`, {
    headers: {
      Authorization: `Bearer ${currentToken}`
    }
  }).then((res) => {
